...
rational_rules <- function(r_idx, game_summary) {
...
```{r}
rule_idxs <- 0:8
results <- do.call(rbind, lapply(rule_idxs, rational_rules, game_summary=game_summary))
plot_results(results)
```
Here prior on outliers is Math.exp(-15), 70000 burn, and 2000 samples.

Here the prior on outliers is Meth.exp(-2), 70000 burn, and 2000 samples.
```{r}
rule_idxs <- 0:8
results <- do.call(rbind, lapply(rule_idxs, rational_rules, game_summary=game_summary))
plot_results(results)
...
Here the prior on outliers is Math.exp(-5), 100000 burn, and 20000 samples.
```{r}
rule_idxs <- 0:0
results <- do.call(rbind, lapply(rule_idxs, rational_rules, game_summary=game_summary))
plot_results(results)
```
Here the prior on outliers is Math.exp(-5), 100000 burn, 20000 samples, lag of 10.
```{r}
rule_idxs <- 0:0
results <- do.call(rbind, lapply(rule_idxs, rational_rules, game_summary=game_summary))
plot_results(results)
