...
rational_rules <- function(r_idx, game_summary) {
  ...
  train_human_responses <- purrr::map(temp$game_id, function(game_id) {
    file.path(base_dir_1, 'train_trials', paste(game_id, '_explorer.csv', sep=''))
  })
  
  test_human_responses <-purrr::map(temp$game_id, function(game_id) {
    file.path(base_dir_1, 'test_trials', paste(game_id, '_explorer.csv', sep=''))
  })
  
  df = data.frame(human_predictives_fp, training_stim_fp, test_stim_fp, train_human_responses, test_human_responses)
  print(df)
  
}

rational_rules(0, game_summary)
```
