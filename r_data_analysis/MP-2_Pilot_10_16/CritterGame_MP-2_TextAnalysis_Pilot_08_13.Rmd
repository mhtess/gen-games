...
df.test.trials <- df.test %>%
  group_by(dyad_id, trialNum, genus, categoryPluralLabel, conceptNumber, featureOrder, role,
           tested_on) %>%
...
df.chat.test <- left_join(
  left_join(
    ...
    df.test.trials %>% ungroup() %>% select(-featureOrder, -n_correct) %>%
      rename(categoryTested = categoryPluralLabel,
             genusTested = genus,
             conceptTested = conceptNumber)
    ),
  df.train.trials %>%
    select(dyad_id, trialNum, role, genus, conceptNumber, categoryPluralLabel) %>%
...
df.chat.test %>% 
  select(dyad_id, trialNum,
        ...
        role,
         text,
         tested_on,
         genusTested, categoryTested, conceptTested, 
         prop_correct
         ) %>% kable()
```
