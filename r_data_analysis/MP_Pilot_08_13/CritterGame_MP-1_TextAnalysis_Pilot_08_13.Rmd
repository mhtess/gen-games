...
text_data <- left_join(text_data, distribution_train_condition, by=c("gameid","trialNum","role"))
...
text_byPartnerScore <- text_df %>%
  ...
  #mutate(allText = paste(text, collapse=" $ ")) %>%
  #unique() %>%
  select(distribution, gameid_num, trialNum, role, text, partnerPercentCorrect, partnerPercentCorrectlyHit, partnerPercentCorrectlyRejected) %>%
  ...
  rename(dyad = gameid_num, totalCorrect = partnerPercentCorrect, hit = partnerPercentCorrectlyHit, CR = partnerPercentCorrectlyRejected)
...
left_join(
  text_byPartnerScore %>%
    ...
    unique(),
  text_byPartnerScore %>%
    select(dyad, trialNum, role, distribution, text) %>%
    group_by(dyad, trialNum, role, distribution) %>%
