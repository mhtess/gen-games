...
data_1 <- read.delim("../logTest/2017-8-11-16h-2m-26s-7270-103c2178-9638-4a00-9ca6-5ed29429346a.csv", header=TRUE)
data_2 <- read.delim("../logTest/2017-8-13-11h-36m-21s-0092-4cf18622-eb4d-46de-9e81-4bedec469478.csv", header=TRUE)
#data_3 <- read.delim("../logTest/2017-8-13-12h-25m-9s-7133-1d5911cf-3f2a-4997-a39d-f565df4b3126.csv", header=TRUE)
data_4 <- read.delim("../logTest/2017-8-13-13h-36m-41s-7480-f3534652-26df-4fb1-848f-859a2cc64c33.csv", header=TRUE)
#data_5 <- read.delim("../logTest/2017-8-13-15h-27m-8s-7735-8d70b20e-2b53-4895-92ee-41e98df7fb15.csv", header=TRUE)
data_6 <- read.delim("../logTest/2017-8-14-10h-37m-22s-3416-c018ab1b-f022-41c4-893c-462e12108278.csv", header=TRUE)
data_7 <- read.delim("../logTest/2017-8-14-12h-22m-40s-7885-73f4a3d7-c242-46d0-96f2-a0c9fb4c2307.csv", header=TRUE)
data_8 <- read.delim("../logTest/2017-8-14-12h-37m-51s-4168-ead62023-8119-4366-8c05-e2091e126643.csv", header=TRUE)

condition_data <- bind_rows(data_1,data_2,data_4,data_6,data_7,data_8)
#condition_data <- bind_rows(data_1,data_2,data_3,data_4,data_6,data_7,data_8)

condition_data$trialNum <- condition_data$trialNum - 1

distribution_condition <- condition_data %>% 
...
distribution_condition$trialNum <- distribution_condition$block_num + 1

text_data <- left_join(text_data, distribution_condition, by=c("gameid","trialNum","role"))


condition_data$partner <- as.factor(
  ifelse(condition_data$role=="playerA", "playerB", "playerA"))
condition_data$hit <- as.factor(
  ifelse(condition_data$score=="hit", 1, 0))
condition_data$correctRejection <- as.factor(
  ifelse(condition_data$score=="correctRejection", 1, 0))
condition_data$falseAlarm <- as.factor(
  ifelse(condition_data$score=="falseAlarm", 1, 0))
condition_data$miss <- as.factor(
  ifelse(condition_data$score=="miss", 1, 0))
condition_data$correct <- as.factor(
  ifelse(condition_data$score=="hit" | condition_data$score=="correctRejection", 1, 0))

roundScore <- condition_data %>%
...
roundScore_playerA <- roundScore %>%
  ...
  select(gameid, trialNum, role, playerA_Hit, playerA_CorrectRejection, playerA_CorrectCount, playerA_PercentCorrectlyHit, playerA_PercentCorrectlyRejected, playerA_PercentCorrect)
...
roundScore_playerB <- roundScore %>%
  ...
  select(gameid, trialNum, role, playerB_Hit, playerB_CorrectRejection, playerB_CorrectCount, playerB_PercentCorrectlyHit, playerB_PercentCorrectlyRejected, playerB_PercentCorrect)

roundScore_combined <- left_join(roundScore, roundScore_playerA, by=c("gameid","trialNum","role"))
roundScore_combined <- left_join(roundScore_combined, roundScore_playerB, by=c("gameid","trialNum","role"))
...
roundScore <- left_join(roundScore, roundScore2, by=c("gameid","trialNum"))
...
roundScore <- roundScore %>%
  ...
  select(trialNum, gameid, role, hitCount, correctRejectionCount, correctCount, percentCorrectlyHit, percentCorrectlyRejected, percentCorrect, partnerHit, partnerCorrectRejection, partnerCorrectCount, partnerPercentCorrectlyHit, partnerPercentCorrectlyRejected, partnerPercentCorrect)
  
  
text_data <- left_join(text_data, roundScore, by=c("gameid","trialNum","role"))

numGames = length(unique(text_data$gameid))
text_data <- text_data %>%
...
text_df$text <- gsub("posionus", "poisonous", text_df$text)
...
wordTypeDictionary <- list(
  ...
  creatures = c("birds","bird","bugs","bug","trees","tree","fish","animals","animal"),
  features = c("properties","property","creatures","creature","critters","critter","colors","color"),
...
text_byPartnerScore <- text_df %>%
  select(gameid, trialNum, distribution, text, partnerPercentCorrect)

kable(text_byPartnerScore)
...
mostFreq_byDistr <- ggplot(count_context_words_byDistr, aes(x=reorder(word, n), y=n)) +
  geom_bar(stat="identity", fill="black") +
  scale_x_discrete("Word Used in Response") +
  scale_y_continuous("Counts", expand=c(0,0)) +
