...
catch_test_reduced_species_by_color <- catch_test_reduced %>%
  group_by(workerid, trial_num, species, distribution, Response.cat, Color) %>%
  distinct() %>%
  summarise(Total = 1) %>%
...
graph_species_by_color <- ggplot(catch_test_reduced_species_by_color, aes(x=Response.cat, y=Total_trials)) +
  geom_bar(stat="identity") +
  scale_x_discrete("Species") +
