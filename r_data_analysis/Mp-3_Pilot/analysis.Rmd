...
teachers_training_labels = plyr::ldply(
  lapply(
    summary_data_reduced$training_trials,
    function(x){ return(data.frame(fromJSON(x)))}
  ), data.frame
) %>%
...
```{r}
training_trial_predictives <- summary_data_reduced$training_trials
```

```{r}
teacher_preds <- webppl(program_file = "./rational_rules.wppl", packages=c("webppl-json"), data=teachers_combined_labels, data_var="combinedTeacherLabels")
