...
temp <- list.files(
  "../../../mturk/mp-game-3/experiment/results-cleaned/train_summary_stats",
  pattern="*.csv",
  full.names=TRUE
)
...
temp <- list.files(
  "../../../mturk/mp-game-3/experiment/results-cleaned/test_summary_stats",
  pattern="*.csv",
  full.names=TRUE
)
...
# Load Chat Logs
temp <- list.files(
  "../../../mturk/mp-game-3/experiment/results-cleaned/chat_messages",
  pattern="*.csv",
  full.names=TRUE
)
...
<!-- Here the prior on outliers is Meth.exp(-5), 20000 burn, and 2000 samples. ```{r} -->
<!-- rule_idxs <- 0:8 results <- do.call(rbind, lapply(rule_idxs, rational_rules, -->
<!-- game_summary=game_summary, incremental=FALSE, num_train=-1))  -->
<!-- plot_results(results) ``` ```{r} train <- results %>% filter(trial_num < 52)  -->
<!-- plot_results(train) ``` -->


<!-- ```{r} test <- results %>% filter(trial_num >= 52) plot_results(test) ``` -->


<!-- ```{r} rule_idxs <- 0:0 results <- do.call(rbind, lapply(rule_idxs, -->
