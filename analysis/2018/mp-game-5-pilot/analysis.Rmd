...
chat_messages_word_count = chat_messages %>%
  ...
  summarise(
    ...
    messageCharLength = sum(nchar(text)),
  )
```
...
```{r}
  ggplot(chat_messages_word_count, aes(x = reorder(role, desc(role)), y = messageCharLength, fill = messageType)) + 
  geom_bar(stat = 'identity', position = 'stack') + 
  ggtitle("# of characters produced by each player, according to utterance category") + 
  xlab('Role') + ylab('# of player\'s characters')
```
