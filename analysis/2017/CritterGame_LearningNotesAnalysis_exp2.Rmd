---
title: "Critter Game - Data - Learning Trial Analysis - Experiment 2"
author: "Lauren Oey"
date: "7/10/2017"
...
text_data <- read.csv("game-2-catch_trials.csv")
...
text_df <- data_frame(line = 1:576, condition=text_data[,6], distribution=text_data[,15], species=text_data[,3],text=text_data[,21])
...
wordTypeDictionary <- list(
  ...
  species_types = c("wug","wugs","fep","feps","lorch","lorches"),
...
tokenized %>%
  group_by(condition, distribution, wordType) %>%
  summarize(n = sum(n))
  
 #convert sentence string to tokens
#tokenized
...
count_all_words_byCond <- tokenized
count_all_words <- tokenized_ungrouped
```
...
species <- count_all_words_byCond %>%
  filter(word %in% species_types) %>%
  group_by(word) %>%
  summarize(n = sum(n))

species_count <- ggplot(species, aes(x=word, y=n)) +
  geom_bar(stat="identity", fill="black") +
