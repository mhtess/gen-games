...
```
```{r}
# Compute how many games are required to hit to target
oustanding_games <- scores_all_games %>%
  count(rule_idx) %>%
  mutate(n = ceiling(n/2)) %>%
  filter(n < 10) %>%
  mutate(num_remaining = 10 - n)
```
...
## Student vs. Teacher Performance Concepts
...
```{r}
# Place explorer and student values side by side
test_all_games_split<- test_all_games %>%
  group_by(stim_num, rule_idx, gameid) %>%
  filter(role == "explorer") %>%
  select(stim_num, rule_idx, gameid, turker_label) %>% 
  rename(explorer_selection = turker_label) %>%
  inner_join(., test_all_games %>%
               filter(role == "student") %>%
               select(stim_num, rule_idx, gameid, turker_label) %>%        
               rename(student_selection = turker_label)
  )
