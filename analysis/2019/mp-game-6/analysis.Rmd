...
ggplot(
  ...
  aes(rule_type_text, color="black", alpha=Role, fill=rule_type_text, y=empirical_stat)
) + 
...
ggplot(complete_test_summary
       , aes(x=student_acc, y=explorer_acc, fill=rule_type_text, color="black", label=rule_idx)) +
  geom_point(size = 10, shape=21) +
  theme_few() +
  guides(fill = guide_legend("Rule"), color=FALSE) +
  geom_point(data = complete_test, size = 2, alpha=0.7, shape=21, show.legend=FALSE) +
  labs(
    x = "Student Accuracy",
    y = "Teacher Acccuracy"
  ) +
...
ggplot(comparison_acc_hd
       , aes(x=explorer_acc, y=mean_d, fill=rule_type_text, color="black")) +
...
ggsave("./analyses/dist-by-acc.png", plot = last_plot(), width = 12, height = 4, dpi = 300 )
```
