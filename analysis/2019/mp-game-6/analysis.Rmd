...
ggplot(complete_test_summary, aes(x=explorer_acc, xend=student_acc, y=reorder(rule_idx, desc(rule_idx)))) + 
  ...
  geom_point(aes(x=chance_rate, y=reorder(rule_idx, desc(rule_idx)), shape=15), size=3, inherit.aes=F, show.legend = F, color="orange") +
...
```{r fig.width=7, fig.height=4, echo=FALSE}
ggplot(avg_complete_test_miss_rate_concept, aes(rule_type_text, fill=role, y=empirical_stat)) + 
  geom_col(position='dodge') +  
  labs(
    title = "Concept Type vs. Miss Rate",
    x = "Concept Type",
    y = "Miss Rate"
  ) + 
  geom_errorbar(aes(x=rule_type_text, ymin=ci_lower, ymax=ci_upper), color="black", width=0.5, position=position_dodge(.9)) +
...
complete_test_false_positive_rate <- scores_complete_games %>%
  mutate(false_positive_rate = false_alarms / (correct_rejections + false_alarms), rule_type = floor(rule_idx/10))
...
```{r fig.width=7, fig.height=4, echo=FALSE}
ggplot(avg_complete_test_false_positive_concept, aes(rule_type_text, fill=role, y=empirical_stat)) + 
  geom_col(position='dodge') +  
  labs(
    title = "Concept Type vs. False Positive Rate",
    x = "Concept Type",
...
avg_gold_hamming_distances <- gold_hamming_distances %>%
  mutate(rule_type = floor(rule_idx/10)) %>%
  group_by(rule_type) %>%
  tidyboot_mean(col=d) %>%
  mutate(type="gold")

# Compute Concept Rule Shuffled Hamming Distances
# Here explorer selections are shuffled amongst one another
# for stimuli of a given concept rule.
complete_all_games_wide_shuffled <- data.frame()
for (si in seq(1000)) {
  for (ri in (seq(50) - 1)) {
    for (sn in (seq(50) - 1)) {
      col <- filter(complete_all_games_wide, stim_num == sn, rule_idx == ri)
      col$explorer <- col[sample(1:nrow(col)), "explorer"]
      col <- col %>%
        mutate(shuffle_index = si)
      complete_all_games_wide_shuffled <- bind_rows(complete_all_games_wide_shuffled, col)
    }
  }
}

concept_rule_shuffled_hamming_distances <- complete_all_games_wide_shuffled %>%
  group_by(rule_idx, gameid, shuffle_index) %>%
  summarize(d = sum(explorer != student))

  avg_concept_rule_shuffled_hamming_distances <- concept_rule_shuffled_hamming_distances %>%
  mutate(rule_type = floor(rule_idx/10)) %>%
  group_by(rule_type, shuffle_index) %>%
  tidyboot_mean(col=d) %>%
  mutate(type="rule_shuffled")

# Compute Null Hamming Distance Distribution
# Here explorer selections are shuffled amongst one another
# across the entire dataset.
complete_all_games_wide_null <- data.frame()
for (si in seq(1000)) {
  complete_all_games_wide_null <- bind_rows(complete_all_games_wide_null,
    complete_all_games_wide %>%
      transform(explorer = sample(explorer)) %>%
      mutate(shuffle_index = si)
  )
}
complete_all_games_wide_null_hamming_distances <- complete_all_games_wide_null %>% 
  group_by(rule_idx, gameid, shuffle_index) %>%
  summarize(d = sum(explorer != student))

avg_complete_all_games_wide_null_hamming_distances <- complete_all_games_wide_null_hamming_distances %>%
  mutate(rule_type = floor(rule_idx/10)) %>%
  group_by(rule_type, shufle_index) %>%
  tidyboot_mean(col=d) %>%
  mutate(type="null")
...
```{r}
ggplot(avg_hamming_distances, aes(x=empirical_stat, color=type, fill=type)) +
  geom_histogram(aes(y=..density..),  position="dodge", binwidth=2)+
  theme(legend.position="right") +
  geom_density(alpha=.2) +
  theme_few() +
  labs(
    title = "Distribution of Hamming Distances",
    x = "Average Hamming Distance for Samples of a Concept Rule",
    y = "Density"
  )
```
