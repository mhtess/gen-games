...
View(complete_test_summary)
cor.test(complete_test_summary$explorer_acc, complete_test_summary$student_acc)
...
hamming_distances <- complete_all_games_wide %>%
  ...
  mutate(rule_type = floor(rule_idx/10), Distrubution="Empirical") 
...
```{r}
# Permute Teacher-Student Pairs Within Concepts
compute_permutation_hd <- function() {
  df <- data.frame()
  for (i in 0:49) {
    ...
    for (row in 1:nrow(x)) {
        x[row, "hd"] = sum(x$explorer[[row]] != x$student[[row]])
    }
    df <- bind_rows(df, x)
  }
  return (df)
...
for (i in 1:1) {
  concept_shuffled_hd <- bind_rows(concept_shuffled_hd, compute_permutation_hd())
}

concept_shuffled_hd$rule_type <- as.factor(concept_shuffled_hd$rule_type)
concept_shuffled_hd <- concept_shuffled_hd %>%
    mutate(rule_type_text = 
           fct_recode(rule_type,
              "Single " = "0",
              "Conj." = "1",
           ...
           Distribution="Intra-Concept Shuffled"
    )
```

```{r}
ggplot(concept_shuffled_hd, aes(x=hd)) +
  geom_histogram(aes(y=(..count..)/tapply(..count..,..PANEL..,sum)[..PANEL..]), bins = 40)+
  theme_few() +
  labs(
    title = "Permuted Teacher & Student Pairs",
    x = "Hamming Distance",
    y = "Probability Mass"
  ) +
...
```{r}
# Combined Hstogram
ch <- bind_rows(hamming_distances, concept_shuffled_hd)
...
hd_samples <- hd_samples %>% mutate(type='True')
```
...
mutate(type='Intra-Concept Shuffle')
```
...
combined_hd_samples <- combined_hd_samples %>%
  ...
  mutate(Distribution = 
           factor(Distribution, levels = c("True", "Intra-Concept Shuffle", "Random Shuffle")
  ))
...
ggplot(explorer_msg_counts, aes(x = uttClassText, y = n)) + 
  labs(
    x = "",
    y = "# of Messages", 
    title = "Teacher: Informative & Follow-Up Messages"
  ) + geom_col() +
  theme(
...
```{r}
ggplot() + 
geom_linerange(data=complete_games_avg_times, mapping=aes(x=Learning, ymin=ci_lower, ymax=ci_upper), size=1) +
geom_point(data=complete_games_avg_times, mapping=aes(x=Learning, y=empirical_stat), size=4, shape=21, fill="white") +
