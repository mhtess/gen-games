...
complete_all_games_wide_shuffled <- complete_all_games_wide
for (ri in (seq(50) - 1)) {
  for (sn in (seq(50) - 1)) {
    col <- filter(complete_all_games_wide_shuffled, stim_num == sn, rule_idx == ri)
    random.explorer <- col[sample(1:nrow(col)), "explorer"]
    complete_all_games_wide_shuffled <- complete_all_games_wide_shuffled %>%
      mutate(explorer = replace(explorer, which(rule_idx == ri & stim_num == sn), random.explorer))
  }
}
...
complete_all_games_wide_null <- complete_all_games_wide
complete_all_games_wide_null <- transform(complete_all_games_wide_null, explorer = sample(explorer))
