...
```{r fig.width=7, fig.height=4, echo=FALSE}
ggplot(complete_test_summary, aes(x=explorer_acc, xend=student_acc, y=reorder(rule_idx, desc(rule_idx)))) + 
  theme_few() +
  geom_segment(
    aes(
      ...
      x=explorer_acc, 
      xend=student_acc, 
      y=reorder(rule_idx, desc(rule_idx)), 
      yend=reorder(rule_idx, desc(rule_idx))),
    size=2) +
  geom_dumbbell(
    size=0,
    size_x=4,
    ...
    labs(
      x="Accuracy",
      y="Concept Rule",
      title="50 Concept Rules: Average Accuracy & Base Rates") +
  geom_point(aes(x=chance_rate, y=reorder(rule_idx, desc(rule_idx)), shape=15), size=4, inherit.aes=F, show.legend = F, color="orange") +
...
## Miss Rate Across Concept Types
```{r}
complete_test_miss_rate <- scores_complete_games %>%
  mutate(miss_rate = misses / (hits + misses), rule_type = floor(rule_idx/10))
complete_test_miss_rate$rule_type <- as.factor(complete_test_miss_rate$rule_type)
complete_test_miss_rate <- complete_test_miss_rate %>%
  mutate(rule_type_text = fct_recode(rule_type,
    "Single" = "0",
    "Conj." = "1",
    "Disj." = "2",
...
complete_test_miss_rate$role <- as.factor(complete_test_miss_rate$role)
complete_test_miss_rate <- complete_test_miss_rate %>%
  mutate(role = fct_recode(role,
    "Teacher" = "explorer",
    "Student" = "student"
...
avg_complete_test_miss_rate <- complete_test_miss_rate %>%
    select(role, gameid, rule_idx, rule_type, rule_type_text, miss_rate) %>%
    group_by(role, rule_idx, rule_type, rule_type_text) %>%
    summarize(avg_miss_rate = mean(miss_rate))

avg_complete_test_miss_rate_concept <- avg_complete_test_miss_rate %>%
    group_by(rule_type, role, rule_type_text) %>%
    tidyboot_mean(avg_miss_rate) %>%
    rename(Role = role)

plot.miss_rate <- ggplot(avg_complete_test_miss_rate_concept, aes(rule_type_text, fill=Role, y=empirical_stat)) + 
  geom_col(position='dodge') +  
  labs(
    x = "Concept Type",
    y = "Miss Rate"
  ...
  theme_few() + 
  scale_y_continuous(
    breaks = c(0.0, 0.25, 0.50, 0.75),
  ) +
  coord_cartesian(ylim=c(0, 0.75))
```

## False Positive Rate Across Concept Types
```{r}
complete_test_false_positive_rate <- scores_complete_games %>%
  mutate(
    false_positive_rate = false_alarms / (correct_rejections + false_alarms),
    rule_type = floor(rule_idx/10)
)

complete_test_false_positive_rate$rule_type <- as.factor(complete_test_false_positive_rate$rule_type)

complete_test_false_positive_rate <- complete_test_false_positive_rate %>%
    mutate(rule_type_text = 
           fct_recode(rule_type,
              "Single" = "0",
              "Conj." = "1",
...
avg_complete_test_false_positive_rate <- complete_test_false_positive_rate %>%
    select(role, gameid, rule_idx, rule_type, rule_type_text, false_positive_rate) %>%
    group_by(role, rule_idx, rule_type, rule_type_text) %>%
    summarize(avg_false_positive_rate = mean(false_positive_rate))

avg_complete_test_false_positive_concept <- avg_complete_test_false_positive_rate %>%
    group_by(rule_type, role, rule_type_text) %>%
    tidyboot_mean(avg_false_positive_rate)

plot.fp_rate <- ggplot(avg_complete_test_false_positive_concept, aes(rule_type_text, fill=role, y=empirical_stat)) + 
  geom_col(position='dodge') +  
  labs(
    x = "Concept Type",
    y = "FP Rate"
...
## Combine False Alarm and Miss Rate Plots
```{r}
...
bootstrap_sample_hd <- function(){
  hamming_distances %>%
  mutate(rule_type = floor(rule_idx/10)) %>%
  group_by(rule_type) %>%
  sample_n(103, replace = T) %>%
  group_by(rule_type) %>%
  summarize(mean_hd = mean(d))
}

# Get 1000 Samples
hd_samples <- data.frame()
for (i in 1:1000) {
  hd_samples <- hd_samples %>% bind_rows(., bootstrap_sample_hd())
}
hd_samples <- hd_samples %>% mutate(type='dyads')
```


```{r}
# Here explorer selections are shuffled amongst one another
# for stimuli of a given concept rule.
bootstrap_concept_rule_shuffle_sample_hd <- function() {
  x <- complete_all_games_wide %>%
    group_by(stim_num, rule_idx, rule_type) %>%
    summarise(mean_stim_hd = mean(sample(explorer) != student)) %>%
    group_by(rule_idx, rule_type) %>%
    summarise(mean_concept_hd = sum(mean_stim_hd)) %>%
    group_by(rule_type) %>%
    summarise(mean_hd = mean(mean_concept_hd))
  return(x)
}

concept_rule_shuffle_hd_samples <- data.frame()
for (i in 1:1000) {
  concept_rule_shuffle_hd_samples <- concept_rule_shuffle_hd_samples %>% bind_rows(bootstrap_concept_rule_shuffle_sample_hd())
}

concept_rule_shuffle_hd_samples <- concept_rule_shuffle_hd_samples %>%
mutate(type='dyads_shuffled_in_concept')
```

```{r}
# Compute Null Hamming Distance Distribution
# Here explorer selections are shuffled amongst one another
# across the entire dataset.
bootstrap_null_sample_hd <- function() {
  x <- complete_all_games_wide %>%
    transform(explorer = sample(explorer)) %>%
    group_by(stim_num, rule_idx, rule_type) %>%
    summarise(mean_stim_hd = mean(explorer != student)) %>%
    group_by(rule_idx, rule_type) %>%
    summarise(mean_concept_hd = sum(mean_stim_hd)) %>%
    group_by(rule_type) %>%
    summarise(mean_hd = mean(mean_concept_hd))  
...
null_hd_samples <- data.frame()
for (i in 1:1000) {
  null_hd_samples <- null_hd_samples %>% bind_rows(bootstrap_null_sample_hd())
} 
null_hd_samples <- null_hd_samples %>%
  mutate(type="null")
```

```{r}
combined_hd_samples <- bind_rows(hd_samples, null_hd_samples, concept_rule_shuffle_hd_samples)
  
combined_hd_samples$rule_type <- as.factor(combined_hd_samples$rule_type) 
combined_hd_samples <- combined_hd_samples %>%
    mutate(rule_type_text = fct_recode(rule_type,
    "Single" = "0",
    "Conj." = "1",
    "Disj." = "2",
  ...
  mutate(Distribution = fct_recode(Distribution,
    "Dyads" = "dyads",
    "Dyads Shuffled (Within Concept)" = "dyads_shuffled_in_concept",
    "Null" = "null",   
  ))
...
```{r}
ggplot(combined_hd_samples, aes(x=mean_hd, color=Distribution, fill=Distribution)) +
  geom_histogram(position="dodge", aes(y=(..count..)/tapply(..count..,..PANEL..,sum)[..PANEL..]), bins = 40)+
  theme(legend.position="right") +
  #geom_density(alpha=.2) +
  theme_few() +
  labs(
    title = "Distribution of Average Hamming Distances",
...
ggsave("./analyses/hamming_distance_density.png", plot = last_plot(), width = 7.5, height = 3, dpi = 300,)
```


```{r}
# Combine Hamming Distances
avg_hamming_distances <- avg_gold_hamming_distances %>% 
  bind_rows(avg_concept_rule_shuffled_hamming_distances, avg_complete_all_games_wide_null_hamming_distances)
avg_hamming_distances$type <- as.factor(avg_hamming_distances$type)
avg_hamming_distances$rule_type <- as.factor(avg_hamming_distances$rule_type)
avg_hamming_distances <- avg_hamming_distances %>%
    mutate(rule_type_text = fct_recode(rule_type,
    "Single" = "0",
    "Conj." = "1",
    "Disj." = "2",
...
```{r}
ggplot(avg_hamming_distances, aes(rule_type_text, fill=type, y=empirical_stat)) + 
  geom_col(position='dodge') +  
  labs(
    title = "Concept Type vs. Average Hamming Distance",
    x = "Concept Type",
    y = "Hamming Distance"
  ) + 
  geom_errorbar(aes(x=rule_type_text, ymin=ci_lower, ymax=ci_upper), color="black", width=0.5, position=position_dodge(0.9)) +
  theme_few()
ggsave("./analyses/avg_hamming_distances_concept_type.png", plot = last_plot())
```
...
complete_games_avg_train_times <- time_complete_games %>%
  ...
  mutate(phase="l", rule_type = floor(rule_idx/10))
...
```{r}
ggplot() + 
geom_linerange(data=complete_games_avg_times, mapping=aes(x=phase, ymin=ci_lower, ymax=ci_upper), width=0.2, size=1) + 
geom_point(data=complete_games_avg_times, mapping=aes(x=phase, y=empirical_stat), size=4, shape=21, fill="white") +
theme_few() + 
facet_wrap(~rule_type, 1) +
  labs(
    title = "Entire Commmunication vs. Training Times Across Concept Rule Types",
    x = "Game Phase",
    y = "Time (Seconds)"
  )
```
...
```{r}
ggplot() + 
geom_linerange(data=complete_games_avg_times, mapping=aes(x=phase, ymin=ci_lower, ymax=ci_upper), size=1, position=position_dodge()) +
geom_point(data=complete_games_avg_times, mapping=aes(x=phase, y=empirical_stat), size=4, shape=21, fill="white") +
geom_line(data=complete_games_avg_times, aes(x=phase, y=empirical_stat, group=rule_idx)) + 
theme_few() + 
facet_wrap(~rule_type, 1) +
  labs(
    title = "Informative & Follow Up Commmunication vs. Training Times Across Concept Rule Types",
    x = "Game Phase",
    y = "Time (Seconds)"
  )
```
