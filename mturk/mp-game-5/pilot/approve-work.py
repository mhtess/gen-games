...
bonus_multiplier = 1 # how many cents is a point worth in this game?
BONUS_DIR = 'bonus'
...
for rfile in results:
    if rfile != '.DS_Store':
        ...
        with open(full_path,'r') as handle:
            ...
            try : 
                bonus = parsed['answers']['bonus']/100.0
            except :
                bonus = 0
            print(bonus)
            d.append({'bonus': bonus,
                    'wID':parsed['WorkerId'],
...
assert sum(np.array(df['bonus'])>200)==False
