function make_slides(f) {
  ...
  slides.one_slider = slide({
    ...
    present_handle : function(stim) {
      ...
      for (var i=0; i<3; i++) {
           $("#crit").append(
            "<svg id='" + stim.subject + i.toString() +
            "'></svg>");

           Ecosystem.draw(
            stim.subject, {},
            "" + stim.subject + i.toString(), scale);
      }
    ...
    button : function() {
      ...
      // Hide the previous svgs after user uses slider and before moving to the next page
      else {
        this.log_responses();
        // make sure you save this in the present_handle above
        var name = this.stim.subject;
        for (var i = 0; i <3; i++) {
          $("#" + name + i.toString()).hide();
        }
...
function init() {
  ...
  exp.structure=["instructions", "critter_page", "one_slider", "i0", "single_trial", "multi_slider", "vertical_sliders", 'subj_info', 'thanks'];
